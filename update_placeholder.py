...
content = content.replace('Llama.cppのサーバーやvLLMなど、OpenAI互換の/v1エンドポイントURLを指定します。', 'Llama.cppのサーバーやvLLMなど、OpenAI互換の/v1エンドポイントURLを指定します。<br><span style="color:var(--accent);">※Docker上で動かしている場合、母艦のMac/PCに接続するには <code>http://host.docker.internal:8080/v1</code> などを指定してください。</span>')
