...
設定済みのプロバイダが初期選択され、未設定のプロバイダは選択できません。URLパイプラインと概念蒸留には別々の選択欄があり、それぞれのジョブに選択値が保存されます。WP-Cronなどログインユーザーが存在しない非同期処理でも、ジョブ所有者のAPIキー、モデル、接続先URLを使用します。
...
### 3.1. URL本文を概念蒸留の根拠にする

URLパイプラインと概念蒸留は別々のジョブですが、概念蒸留の「根拠資料（任意）」で、URLパイプラインが抽出した本文を関連付けられます。

1. 先に対象URLをURLパイプラインへ登録します。
2. `extraction`（文章抽出）が完了するまで待ちます。
3. 概念蒸留で中心概念とLLMを選びます。
4. 「根拠資料」から対象記事を選択して開始します。

関連付けた本文は、概念マップ、枝ごとの質問、複数回答の生成時に根拠として渡されます。URLジョブが後続の`summary`で停止していても、文章抽出済みなら根拠資料として利用できます。文章抽出が未完了の場合、概念ジョブは完了を待機します。
...
処理キューのエラー行には原因と「この工程から再実行」が表示されます。API設定やプロバイダを修正した後に押すと、保存済みの本文や中間結果を維持したまま、失敗したステージから再開します。概念蒸留では、画面上部で選択したLLMと根拠資料が再実行時に反映されます。
...
「学習価値」を開くと、「品質が高いか」に加えて「このデータから何を新しく学べるか」を確認できます。評価は外部APIを呼ばない決定論的な一次評価で、品質再評価、概念グラフ編集、選択的再蒸留の完了時に更新されます。

保存される主な指標は次のとおりです。

- `training_value`: 情報利得、信頼性、概念被覆をまとめた総合参考値
- `information_gain`: `Novelty × Reliability × CoverageGain`を0〜100へ正規化した値
- `novelty`: 他の採用回答と内容が重なっていない程度
- `reliability`: 品質点と回答自身の確信度を組み合わせた値
- `coverage_gain`: その回答が枝・質問の不足を埋める程度
- `redundancy`: 採用回答群との最大類似度
- `difficulty_level`: 単純事実から仮説形成までの1〜7段階
- `contradiction_status`: 同じ質問内で、類似した主張の極性が反対になっている可能性
- `training_eligibility`: `eligible`、`review_required`、`excluded`

概念被覆は、有効な枝のうち採用回答を持つ枝の割合です。「次に埋める知識領域」には、質問数と採用回答数から算出した被覆率が低い枝を最大5件表示します。連続生成の件数を増やす前に、この一覧から不足領域を選ぶ運用を推奨します。

各サンプルには`source_id → knowledge_id → sample_id`の最小系譜を保存します。これにより、元の概念知識と学習用表現を同一視せず、後から本格的なDAG系譜へ移行できます。ライセンスと時間的有効性は自動保証せず、初期状態を`review_required` / `unassessed`として明示します。

矛盾候補は自動削除しません。似た文で肯定・否定が反転している候補を`possible_conflict`として保持し、人間レビューまたは複数LLM審査へ回します。この検出は語彙的な一次検出なので、専門領域の真偽判定には出典・日付・証拠水準の追加が必要です。
...
- [Done] 学習価値・知識被覆・難易度・矛盾候補・最小系譜の一次評価を実装
- [Next] 被覆不足の枝を選び、質問自体を差分確認して追加生成する機能
- [Later] Claim Clusterと出典証拠を使う本格的な矛盾管理
- [Later] Source / Knowledge / Training Material / Dataset Buildの4層分離と系譜DAG
- [Later] Cloud側とはHTTPS APIまたはジョブキューで接続する独立Worker
...
- [Done] 学習価値・知識被覆・矛盾候補・最小系譜の可視化
- [Next] 被覆不足の枝から質問を選択生成する差分プレビュー
